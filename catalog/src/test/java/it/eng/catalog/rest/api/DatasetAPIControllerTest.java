...
public class DatasetAPIControllerTest {
    ...
    public void getFormatsFromDatasetSuccessfulTest() {
        when(datasetService.getFormatsFromDataset(CatalogMockObjectUtil.DATASET_WITH_ARTIFACT.getId()))
                .thenReturn(CatalogMockObjectUtil.DATASET_WITH_ARTIFACT.getDistribution().stream()
                        .map(Distribution::getFormat).collect(Collectors.toList()));
