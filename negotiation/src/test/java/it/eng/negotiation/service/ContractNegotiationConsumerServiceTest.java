...
public class ContractNegotiationConsumerServiceTest {
	...
	public void handleAgreement_off_success() {
		...
		service.handleAgreement(ModelUtil.CALLBACK_ADDRESS, ModelUtil.CONTRACT_AGREEMENT_MESSAGE);
		//TODO fix this test to verify that status is updated to agreed
		ContractNegotiation contractNegtiationUpdate = ContractNegotiation.Builder.newInstance()
				.id(ModelUtil.CONTRACT_NEGOTIATION.getId())
		...
		inOrder.verify(repository, Mockito.atLeastOnce()).findByProviderPidAndConsumerPid(ModelUtil.PROVIDER_PID, ModelUtil.CONSUMER_PID);
		inOrder.verify(repository, Mockito.atLeastOnce()).save(any(ContractNegotiation.class));
	}
