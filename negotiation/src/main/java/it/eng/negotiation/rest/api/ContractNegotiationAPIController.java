...
public class ContractNegotiationAPIController {
    ...
    @GetMapping
    public ResponseEntity<GenericApiResponse<Collection<JsonNode>>> getContractNegotiations(@PathVariable(required = false) String state){
    ...
    public ResponseEntity<GenericApiResponse<JsonNode>> verifyContractNegotiation(@RequestBody JsonNode verifyNegotiationJson) {
        ...
        apiService.verifyNegotiation(consumerPid, providerPid);
    	return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON)
