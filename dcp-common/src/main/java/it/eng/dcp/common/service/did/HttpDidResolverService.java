...
public class HttpDidResolverService implements DidResolverService {
    ...
    /**
     * Converts a did:web DID to an HTTPS URL.
     *
     * @param did The did:web DID
     * @return The HTTPS URL to fetch the DID document
     */
    private String convertDidToUrl(String did) {
        ...
        if (segments.length == 1) {
            ...
            if (secondSegment.matches("\\d+")) {
                // Port: did:web:localhost:8080
                hostPort = segments[0] + ":" + segments[1];
            } else {
                // Path: did:web:example.com:users
                hostPort = segments[0];
                pathSegments = secondSegment;
            }
        } else {
            // 3+ segments: check if second is port
        ...
        if (pathSegments == null || pathSegments.isEmpty()) {
            return "https://" + hostPort + "/.well-known/did.json";
        } else {
            return "https://" + hostPort + "/" + pathSegments + "/did.json";
        }
    }
