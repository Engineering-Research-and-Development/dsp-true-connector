...
class IssuerServiceTest {
    ...
    void approveAndDeliverCredentials_autoGenerate_success() {
        ...
        IssuerMetadata.CredentialObject membershipMeta = IssuerMetadata.CredentialObject.Builder.newInstance()
                .id("meta-1")
                .type("CredentialObject")
                .credentialType("MembershipCredential")
        ...
        IssuerMetadata.CredentialObject companyMeta = IssuerMetadata.CredentialObject.Builder.newInstance()
                .id("meta-2")
                .type("CredentialObject")
                .credentialType("CompanyCredential")
                .profile("vc11-sl2021/jsonld")
                .build();

        IssuerMetadata metadata = IssuerMetadata.Builder.newInstance()
                .issuer("did:web:localhost:8090")
                .credentialsSupported(List.of(membershipMeta, companyMeta))
                .build();

        // Credentials generated by issuance service (with default format)
        List<CredentialMessage.CredentialContainer> generatedCredentials = Arrays.asList(
                CredentialMessage.CredentialContainer.Builder.newInstance()
                        .credentialType("MembershipCredential")
                        .payload(Map.of("membership", "Premium"))
                        .format("jwt")
                        .build(),
                CredentialMessage.CredentialContainer.Builder.newInstance()
                        .credentialType("CompanyCredential")
                        .payload(Map.of("company", "ACME"))
                        .format("jwt")
                        .build()
        );
        ...
        when(requestRepository.findByIssuerPid(requestId)).thenReturn(Optional.of(request));
        when(credentialMetadataService.buildIssuerMetadata()).thenReturn(metadata);
        when(issuanceService.generateCredentials(eq(request), anyMap(), isNull())).thenReturn(generatedCredentials);
        ...
        assertTrue(result.getCredentialTypes().contains("MembershipCredential"));
        assertTrue(result.getCredentialTypes().contains("CompanyCredential"));

        verify(requestRepository).findByIssuerPid(requestId);
        verify(credentialMetadataService).buildIssuerMetadata();
        verify(issuanceService).generateCredentials(eq(request), anyMap(), isNull());

        // Verify that credentials were delivered with correct format from metadata
        verify(deliveryService).deliverCredentials(eq(requestId), argThat(creds -> {
            CredentialMessage.CredentialContainer membershipCred = creds.stream()
                    .filter(c -> "MembershipCredential".equals(c.getCredentialType()))
                    .findFirst().orElse(null);
            CredentialMessage.CredentialContainer companyCred = creds.stream()
                    .filter(c -> "CompanyCredential".equals(c.getCredentialType()))
                    .findFirst().orElse(null);

            return membershipCred != null && "jwt".equals(membershipCred.getFormat()) &&
                   companyCred != null && "jsonld".equals(companyCred.getFormat());
        }));
    }
    ...
    void approveAndDeliverCredentials_emptyGeneratedCredentials_throwsException() {
        ...
        when(requestRepository.findByIssuerPid(requestId)).thenReturn(Optional.of(request));
        when(credentialMetadataService.buildIssuerMetadata()).thenReturn(metadata);
        when(issuanceService.generateCredentials(eq(request), anyMap(), isNull())).thenReturn(Collections.emptyList());
    ...
    @Test
    void approveAndDeliverCredentials_appliesMetadataFormatFromProfile() {
        // Arrange
        String requestId = "req-456";
        CredentialRequest request = CredentialRequest.Builder.newInstance()
                .issuerPid(requestId)
                .holderPid("did:example:holder456")
                .status(CredentialStatus.PENDING)
                .credentialIds(List.of("MembershipCredential"))
                .build();

        // Mock metadata with specific profile
        IssuerMetadata.CredentialObject membershipMeta = IssuerMetadata.CredentialObject.Builder.newInstance()
                .id("meta-1")
                .type("CredentialObject")
                .credentialType("MembershipCredential")
                .profile("vc11-sl2021/jsonld")
                .credentialSchema("https://example.com/schemas/membership.json")
                .build();

        IssuerMetadata metadata = IssuerMetadata.Builder.newInstance()
                .issuer("did:web:localhost:8090")
                .credentialsSupported(List.of(membershipMeta))
                .build();

        // Credential generated with default jwt format
        List<CredentialMessage.CredentialContainer> generatedCredentials = List.of(
                CredentialMessage.CredentialContainer.Builder.newInstance()
                        .credentialType("MembershipCredential")
                        .payload(Map.of("membership", "Gold"))
                        .format("jwt")
                        .build()
        );
        ...
        // Verify that format was changed from jwt to jsonld based on profile
        verify(deliveryService).deliverCredentials(eq(requestId), argThat(creds -> {
            CredentialMessage.CredentialContainer cred = creds.get(0);
            return "MembershipCredential".equals(cred.getCredentialType()) &&
                   "jsonld".equals(cred.getFormat());
        }));
    }

    @Test
    void approveAndDeliverCredentials_noMetadataForCredentialType_usesDefaultFormat() {
        // Arrange
        String requestId = "req-789";
        CredentialRequest request = CredentialRequest.Builder.newInstance()
                .issuerPid(requestId)
                .holderPid("did:example:holder789")
                .status(CredentialStatus.PENDING)
                .credentialIds(List.of("UnknownCredential"))
                .build();

        // Mock metadata without UnknownCredential
        IssuerMetadata.CredentialObject membershipMeta = IssuerMetadata.CredentialObject.Builder.newInstance()
                .id("meta-1")
                .type("CredentialObject")
                .credentialType("MembershipCredential")
        ...
        // Credential generated with default format
        List<CredentialMessage.CredentialContainer> generatedCredentials = List.of(
                CredentialMessage.CredentialContainer.Builder.newInstance()
        ...
        // Verify that credential uses default format when no metadata found
        verify(deliveryService).deliverCredentials(eq(requestId), argThat(creds -> {
            CredentialMessage.CredentialContainer cred = creds.get(0);
            return "UnknownCredential".equals(cred.getCredentialType()) &&
                   "jwt".equals(cred.getFormat());
        }));
    }
