...
**⚠️ Critical:** `dcp.connector.did` is REQUIRED for `SelfIssuedIdTokenService` to generate authentication tokens for credential requests and deliveries.
...
## 📋 Common API Calls (After Connector is Started!)
...
├── service/
│   ├── HolderService.java          # Credential storage
...
├── VcVpAuthenticationFilter.java   # Extract VP from headers
├── VcVpAuthenticationProvider.java # Validate VP
...
**Check logs for:**
- "Profile mismatch" → VCs have different profileIds
- "Credential expired" → Check expiration dates
- "Signature verification failed" → Key mismatch
- "Untrusted issuer" → Add to trusted-issuers
...
**Common Mistake:**
```
❌ did:web:localhost:8080        (Won't resolve correctly)
✅ did:web:localhost%3A8080      (Correct)
```
...
3. Start connector
   └── cd connector && mvn spring-boot:run

4. Wait for startup
   └── "Started ApplicationConnector in X seconds"
...
dcp/src/main/java/it/eng/dcp/
├── rest/
│   ├── DcpController.java          # Holder endpoints
│   └── IssuerController.java       # Issuer endpoints
├── service/
│   ├── HolderService.java
│   ├── PresentationService.java
│   ├── PresentationValidationService.java
│   └── IssuerService.java
└── model/
...
did:web:localhost%3A8080:holder
  → https://localhost:8080/holder/.well-known/did.json
```
