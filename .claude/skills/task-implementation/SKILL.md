...
The Copilot-native canonical version of this workflow lives at `.github/skills/task-implementation/SKILL.md`. Keep this Claude-side file aligned as a compatibility mirror rather than a separate source of truth.

This is the **canonical implementation workflow** for Claude-first execution of Ready-column issues on the GitHub Project board. Keep the detailed issue-selection, implementation, per-task verification, and PR-handoff procedure here. Other active repo docs should point to this file instead of restating the full flow.
...
- The repo-local `task-implementation` skill works by living under `.claude/skills/`; no separate installation step is required for Claude Code.
...
After the PR is open, run a self-review of the branch diff using the Claude Code built-in `/code-review` skill at **high** effort.
