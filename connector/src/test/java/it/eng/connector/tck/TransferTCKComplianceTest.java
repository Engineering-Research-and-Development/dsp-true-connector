...
@ActiveProfiles("tck")
@Disabled("Disabled until the TCK issues are resolved - works correctly wit 8080 port not 8090")
...
//        properties.put("dataspacetck.debug", "true");
        ...
        properties.put("dataspacetck.dsp.connector.transfer.initiate.url", "http://localhost:8090/consumer/transfers/tck");
//        properties.put("dataspacetck.dsp.default.wait", "5000");
        ...
        for (int i = 1; i <= 3; i++) {
            for (int j = 1; j <= 6; j++) {
                ...
                properties.put(prefix + "AGREEMENTID", "ATP0%d0%d".formatted(i, j));
        ...
        for (int i = 1; i <= 3; i++) {
            for (int j = 1; j <= 6; j++) {
                ...
                properties.put(prefix + "AGREEMENTID", "ATPC0%d0%d".formatted(i, j));
                properties.put(prefix + "FORMAT", "HttpData-PULL");
            }
        }
