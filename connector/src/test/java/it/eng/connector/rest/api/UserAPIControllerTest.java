...
class UserAPIControllerTest {
	...
	void updateUserNames() {
		...
		when(userService.updateUserNames(USER_ID, USER, userDTO)).thenReturn(ToolsSerializer.serializePlainJsonNode(TestUtil.API_USER));
		GenericApiResponse<JsonNode> response =  controller.updateUserNames(USER_ID, userDTO, principal).getBody();
		assertNotNull(response);
		assertNotNull(response.getData());
	}
	...
	void updateUser_Names_error() {
		when(principal.getName()).thenReturn(USER);
		doThrow(BadRequestException.class).when(userService).updateUserNames(USER_ID, USER, userDTO);
		assertThrows(BadRequestException.class, ()-> controller.updateUserNames(USER_ID, userDTO, principal));
	}
