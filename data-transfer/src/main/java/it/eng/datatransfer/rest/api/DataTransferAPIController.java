...
public class DataTransferAPIController {
     ...
     * @param state             optional state to filter transfer processes by, if present.
     * @param role              optional role to filter transfer processes by (e.g., CONSUMER or PROVIDER).
     * @param datasetId         optional dataset identifier to filter transfer processes by, if present.
     * @param providerPid       optional provider PID to filter transfer processes by, if present.
     * @param consumerPid       optional consumer PID to filter transfer processes by, if present.
    ...
    public ResponseEntity<GenericApiResponse<Collection<JsonNode>>> getTransfersProcess(
        ...
        log.info("Fetching transfer process id - {}, state {}, role {}, datasetId {}, providerPid {}, consumerPid {}",
                transferProcessId, state, role, datasetId, providerPid, consumerPid);
        Collection<JsonNode> response = apiService.findDataTransfers(transferProcessId, state, role, datasetId, providerPid, consumerPid);
        return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON)
