...
- **AUTH1-4 — Unified backend-mediated authentication & JWT login (#281)**
  ...
  - **Internal-service M2M** — new `InternalServiceTokenIssuer` (`tools`) replaces `InternalAuthenticationService`, minting a correctly-scoped `ROLE_ADMIN` JWT for inter-module calls without embedding `application.internal.secret` in any JWT claim.
