...
- A task adds or updates Java classes under a `model` package.
...
1. Mirror the model conventions used in nearby code before introducing new patterns or abstractions.
2. Keep Jackson, validation, and persistence annotations aligned with the model's actual role in the repository.
3. Update or add model tests when serialization, validation, or builder behavior changes.
