...
The data transfer suites download artifacts via presigned URLs generated by MinIO. The presigned URL hostname is controlled by `s3.externalPresignedEndpoint`, which is injected through `MINIO_EXTERNAL_URL` when the GitHub Actions jobs start `docker compose`.

- For the current `datatransfer-api-http-pull-tests` and `datatransfer-api-http-push-tests` jobs in `build.yml`, the workflow explicitly sets `MINIO_EXTERNAL_URL=http://localhost:9000`.
- That job-level environment variable overrides the fallback value from `ci/docker/.env` when `docker compose` starts the stack.
- To reproduce those jobs locally, export the same value before running compose, for example:
...
- If you run locally without the override, compose may fall back to the `.env` default (`http://172.17.0.1:9000`), and Newman can fail only at the presigned URL download step even though the workflow would pass.
